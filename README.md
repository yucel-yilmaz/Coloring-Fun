...
- Kalem, keçeli kalem, pastel boya ve sprey fırçaları (basınca duyarlı, damga tabanlı boya motoru; Apple Pencil desteği), boya kovası, silgi, geri/ileri al ve galeri desteği olan dokunmatik boyama alanı
- Üyeliksiz kullanılabilen yerel çizim kataloğu; kategoriler içeriğe göre türetilir (boş kategoriler otomatik gizlenir) ve admin panelinden yeni kategori eklenebilir
- Ebeveyne bağlı çocuk profilleri ve yaş grupları
- Gemini, OpenAI, fal.ai, Replicate, Hugging Face, Stability AI ve yerel SDXL sağlayıcıları
...
| `npm run db:migrate` | Bekleyen SQL migration'larını uygular (bkz. Migration'ları uygulama bölümü) |
| `npm run lint` | TypeScript tip kontrolünü çalıştırır |
| `npm test` | Birim ve API testlerini çalıştırır |
| `npm run build` | Web, API, worker, cleanup ve migrate çıktılarını üretir |
...
`dist/cleanup.cjs` zamanlanmış temizleme görevini, `dist/migrate.cjs` ise bekleyen
migration'ları uygulayan aracı içerir. Worker'ı ayrı bir süreçte
`npm run start:worker` ile çalıştırın; cleanup görevini periyodik olarak
`npm run start:cleanup` ile çağırın; migration'ları `npm run start:migrate` ile
uygulayın (`SUPABASE_DB_URL` ister).

## Coolify

Yerel AI kullanmadan yayınlamak için Coolify'da Docker Compose build tipini
seçin. Depodaki `docker-compose.yml` tek kurulumda üç servis başlatır:

| Servis | Açıklama |
| --- | --- |
| `migrate` | Her deploy'da bekleyen migration'ları uygulayıp çıkan bir kerelik iş; `web`/`worker`'ı bloklamaz, `SUPABASE_DB_URL` boşsa no-op geçer |
