...
export function AdminPage() {
  ...
  const removeArtwork = async (item: Artwork) => {
    if (!(await confirm({ message: `“${item.title}” görseli ve dosyaları kalıcı olarak silinsin mi?`, confirmLabel: 'Sil', danger: true }))) return;
  ...
  const removeCatalogPage = async (item: CatalogPage) => {
    if (!(await confirm({ message: `“${item.title || item.nameTr}” Boyama Seç bölümünden silinsin mi?`, confirmLabel: 'Sil', danger: true }))) return;
