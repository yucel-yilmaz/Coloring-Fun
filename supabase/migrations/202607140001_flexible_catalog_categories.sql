...
--
-- Written idempotently (safe to run more than once): both the original auto-named check and the
-- new one are dropped first, then the slug check is (re)created.

alter table public.coloring_page_overrides
  drop constraint if exists coloring_page_overrides_category_check;

alter table public.coloring_page_overrides
  drop constraint if exists coloring_page_overrides_category_slug_check;

alter table public.coloring_page_overrides
  add constraint coloring_page_overrides_category_slug_check
